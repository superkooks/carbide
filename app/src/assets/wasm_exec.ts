...
if (!window.crypto) {
  throw new Error(
    "window.crypto is not available, polyfill required (crypto.getRandomValues only)"
  )
}

if (!window.performance) {
  throw new Error(
    "window.performance is not available, polyfill required (performance.now only)"
  )
}
...
declare global {
  ...
  // interface RxSession {
  //   // All string types are base64
  //   receiveMessage: (msg: string) => string
  // }
}
